...
function getMaxPages(catalog, num) {
  return catalog.filter(book => book.pages <= num).map(book => `${book.title}, pages: ${book.pages}.`).join('\n');
}

function getMinPages(catalog, num) {
  return catalog.filter(book => book.pages > num).map(book => `${book.title}, pages: ${book.pages}.`).join('\n');
}

console.log('\nBooks with less than 300 pages:\n');
console.log(getMaxPages(library, 300));

console.log('\nBooks with more than 300 pages:\n');
console.log(getMinPages(library, 300));
